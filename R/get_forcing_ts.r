...
#' @param path character string. File path to ts files (Default = NULL, implies you are in project folder)
...
#' @examples
#' \dontrun{
#' pull ts file from github
#' get_forcing_ts(path="https://raw.githubusercontent.com/NEFSC/READ-EDAB-neusAtlantis/refs/heads/andy_communities/currentVersion/CatchFiles/total_catch_fleets.ts")
#' }
#'

get_forcing_ts <- function(code=NULL,path=NULL,filenm ="total_catch_fleets",time="daily") {
  ...
  if(is.null(path)) {
    ...
    #file <- paste0(path,"/",paste0(filenm,".ts"))
    file <- path
